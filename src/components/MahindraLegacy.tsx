...
export default function MahindraLegacy() {
  ...
  return (
    <section className="w-full py-12 sm:py-16 lg:py-24 bg-[#0b1120] text-white scroll-mt-20 border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-10 sm:mb-14">
          {legacy.stats.map((stat, idx) => (
            <div
              key={idx}
              className="bg-slate-900/90 rounded-xl p-5 sm:p-6 border border-slate-800 text-center space-y-1.5 shadow-sm"
            >
              <span className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold text-white block">
                {stat.value}
              </span>
              <span className="text-[10px] sm:text-xs text-white font-semibold uppercase tracking-wider block">
                {stat.label}
              </span>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {legacy.verticals.map((vert, idx) => (
            <div
              key={idx}
              className="bg-slate-900/60 rounded-xl p-5 sm:p-6 border border-slate-800/80 space-y-2 shadow-sm"
            >
